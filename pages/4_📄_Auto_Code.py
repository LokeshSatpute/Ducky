...
import helpers.util
from aitools_autogen.blueprint_project8 import CodeSolvingBlueprint
...
if st.session_state.get("blueprint", None) is None:
    st.session_state.blueprint = CodeSolvingBlueprint()
...
with blueprint_ctr:
    ...
    url = st.text_input("Enter the url to the leet-code or hackerrank problem:",
                        value="https://leetcode.com/problems/longest-increasing-path-in-a-matrix/")
...
if agents:
    ...
    task = f"""
            I want to get the solution for the following leet-code or hackerrank problem:
            {url}
            """
